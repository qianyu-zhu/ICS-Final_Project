...
def verify():
    ...
    if float(t) >= 120:      # Check it the user enters above 2 minutes. So i set as >=120
        messagebox.showinfo("Time out", "Session Expired ...Time out Please regenerate password")
        Window.destroy()
    else:
        cmd1=str(a.get())             # Get the entered OTP
        cmd='python verify.py '+cmd1
        os.system(cmd)                # call the verify program
        ok='Invalid password: '+str((count-1))+' attempts remaining'
        count=count-1
        ...
        if count>=1 and bh != "success":
            ...
            f.write("")
            f.close()
            messagebox.showinfo("Oooo","Your 3 attempts was over. Please regenerate password")
            f1.close()
            Window.destroy()
        elif bh == "success":
            f1.close()
...
a=StringVar()
Re=Label(Window,text="Enter your password",font=("arial",15,"bold")).place(x=0,y=50)
w1=Entry(Window,width=20,textvariable=a)
w1.place(x=400,y=50)
Re1=Label(Window,text="Please enter within 2 minutes",font=("arial",10,"bold")).place(x=400,y=85)
ver = Button(Window, text="Verify",relief="raised", bg='blue', font=("arial", 15, "bold"), fg='blue',command=verify).place(x=300,y=150)
Window.mainloop()